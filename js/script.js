document.addEventListener("DOMContentLoaded", function() {
    ...
    let genderInputs = document.querySelectorAll('input[name="radio"]');
    ...
    sub.onclick = function() {
        ...
        if (fname && lname && email && pass && cpass && check) {
            if (pass === cpass) {
                ...
                // Redirect to logged.html with query parameters
                window.location.href = `logged.html?fname=${encodeURIComponent(fname)}
                &lname=${encodeURIComponent(lname)}&gender=${encodeURIComponent(gender)}
